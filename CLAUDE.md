...
- QuestionProgress is written only in src/server/mutations/progress.ts: `applyProgress()` for answers and `resetProgressForQuestion()` for answer-key changes. `applyProgress()` does INSERT ... ON CONFLICT DO NOTHING, then SELECT ... FOR UPDATE ordered by questionId, then one UPDATE, all inside the caller's transaction.
